...
* **Both catalogs:**
  ...
  * Both East and West are using common STAC Transaction API code (with separate configs for auth, Globus v. EGI Check-In).
  * Unclear if we're currently using the same event consumer and STAC Discovery API code or not. (Will clarify during April.)
